...
contract AngstromPair is IAngstromPair, UniswapV2ERC20 {
  ...
  function _safeTransfer(address token, address to, uint value) private {
    ...
    require(success && (data.length == 0 || abi.decode(data, (bool))), 'AngstromPair: TRANSFER_FAILED');
  }
  ...
  function initialize(address _token0, address _token1) external {
    require(msg.sender == factory && !initialized, 'AngstromPair: FORBIDDEN');
    // sufficient check
    token0 = _token0;
    token1 = _token1;
  ...
  */
  function setFeePercent(uint16 newToken0FeePercent, uint16 newToken1FeePercent) external lock {
    require(msg.sender == IAngstromFactory(factory).feePercentOwner(), "AngstromPair: only factory's feeAmountOwner");
    require(newToken0FeePercent <= MAX_FEE_PERCENT && newToken1FeePercent <= MAX_FEE_PERCENT, "AngstromPair: feePercent mustn't exceed the maximum");
    require(newToken0FeePercent > 0 && newToken1FeePercent > 0, "AngstromPair: feePercent mustn't exceed the minimum");
  ...
  function setStableSwap(bool stable, uint112 expectedReserve0, uint112 expectedReserve1) external lock {
    require(msg.sender == IAngstromFactory(factory).setStableOwner(), "AngstromPair: only factory's setStableOwner");
    require(!pairTypeImmutable, "AngstromPair: immutable");

    require(stable != stableSwap, "AngstromPair: no update");
    require(expectedReserve0 == reserve0 && expectedReserve1 == reserve1, "AngstromPair: failed");
  ...
  function setPairTypeImmutable() external lock {
    require(msg.sender == IAngstromFactory(factory).owner(), "AngstromPair: only factory's owner");
  ...
  function _update(uint balance0, uint balance1) private {
    require(balance0 <= uint112(- 1) && balance1 <= uint112(- 1), 'AngstromPair: OVERFLOW');
  ...
  function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
    ...
    (uint ownerFeeShare, address feeTo) = IAngstromFactory(factory).feeInfo();
  ...
  function burn(address to) external lock returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'AngstromPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function _swap(TokensData memory tokensData, address to, bytes memory data, address referrer) internal lock {
    require(tokensData.amount0Out > 0 || tokensData.amount1Out > 0, 'AngstromPair: INSUFFICIENT_OUTPUT_AMOUNT');

    (uint112 _reserve0, uint112 _reserve1, uint16 _token0FeePercent, uint16 _token1FeePercent) = getReserves();
    require(tokensData.amount0Out < _reserve0 && tokensData.amount1Out < _reserve1, 'AngstromPair: INSUFFICIENT_LIQUIDITY');


    {
      require(to != tokensData.token0 && to != tokensData.token1, 'AngstromPair: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'AngstromPair: INSUFFICIENT_INPUT_AMOUNT');
    ...
    {// scope for referer/stable fees management
      ...
      uint referrerInputFeeShare = referrer != address(0) ? IAngstromFactory(factory).referrersFeeShare(referrer) : 0;
    ...
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      ...
      require(_k(balance0Adjusted, balance1Adjusted) >= _k(uint(_reserve0), uint(_reserve1)), 'AngstromPair: K');
    }
  ...
  function sync() external lock {
    ...
    require(token0Balance != 0 && token1Balance != 0, "AngstromPair: liquidity ratio not initialized");
  ...
  */
  function drainWrongToken(address token, address to) external lock {
    require(msg.sender == IAngstromFactory(factory).owner(), "AngstromPair: only factory's owner");
    require(token != token0 && token != token1, "AngstromPair: invalid token");
